...
def update_grafico(frame_num, dados_gantt, ax, cores, processo_para_y, tempo_total):
    ax.clear()
    ax.set_xlim(0, tempo_total)  # Assumindo algum tempo máximo de simulação
    ax.set_ylim(0, len(processo_para_y))  # Ajusta a altura do gráfico para o número de processos
    ...
    ax.set_xticks(np.arange(0, tempo_total + 1, 1))  # Adiciona todas as unidades de tempo no eixo X
    ax.grid(True, which='both', axis='both', linestyle='--', linewidth=0.5)  # Adiciona grid ao gráfico
    ax.set_xlabel('Tempo')
...
def round_robin(processos, quantum, output_file, dados_gantt, ax, cores, processo_para_y):
    ...
    # Inicializa tempos de espera e execução
    ...
    for i in range(tempo_total + 1):
        ...
        # Atualiza os dados do gráfico Gantt
        if len(fila_cpu) > 0:
        ...
        update_grafico(i, dados_gantt, ax, cores, processo_para_y, tempo_total)
        plt.pause(0.1)
        time.sleep(1)

        # Adiciona novos processos à fila de processos
    ...
    # Calcula e imprime tempos de espera
...
def main():
    filePath = '.\\ArquivoEntrada.txt'
    outputPath = '.\\resultado_simulacao.txt'
    processos = ler_arquivo_entrada(filePath)
    quantum = 4
    fig, ax = plt.subplots(figsize=(12, 6))

    # Identificando processos únicos e atribuindo um índice no eixo y
    ...
    # Gerar cores aleatórias para cada processo e armazenar para uso consistente
